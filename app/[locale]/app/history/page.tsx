                ...
                {userQuotes.map((q) => (
                  <div key={q.quotes.id} className="flex items-center justify-between p-6 hover:bg-slate-50 transition">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-bold text-slate-900">{q.clients?.name || 'Cliente'}</p>
                        <p className="text-sm text-slate-500">{new Date(q.quotes.createdAt).toLocaleDateString('es-ES')}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <span className={`rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wider ${
                        q.quotes.status === 'draft' ? 'bg-slate-100 text-slate-600' :
                        q.quotes.status === 'sent' ? 'bg-blue-100 text-blue-700' :
                        q.quotes.status === 'accepted' ? 'bg-green-100 text-green-700' :
                        'bg-red-100 text-red-700'
                      }`}>
                        {q.quotes.status === 'draft' ? 'Borrador' :
                         q.quotes.status === 'sent' ? 'Enviado' :
                         q.quotes.status === 'accepted' ? 'Aceptado' :
                         'Rechazado'}
                      </span>
                      <Link
