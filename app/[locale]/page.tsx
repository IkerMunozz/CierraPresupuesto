...
async function Dashboard({ userId }: { userId: string }) {
  ...
  try {
    recentQuotes = await db.select().from(quotes).leftJoin(clients, eq(quotes.clientId, clients.id)).where(eq(quotes.userId, userId)).orderBy(desc(quotes.createdAt)).limit(5);
        ...
        <div className="mb-10">
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-2 flex items-center justify-between">
              ...
              <Link href="/subscription" className="text-xs font-bold text-slate-500 hover:text-slate-900 transition underline">
                Mejorar plan
              </Link>
            </div>
          </div>
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          <section className="lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/app/history" className="text-xs font-bold text-blue-600 hover:underline">Ver todos</Link>
            </div>
            <div className="rounded-3xl border border-slate-200 bg-white overflow-hidden shadow-sm">
              {recentQuotes.length > 0 ? (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4">
                        <span className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-wider ${
                          q.quotes.status === 'borrador' ? 'bg-slate-100 text-slate-600' : 'bg-green-100 text-green-700'
                        }`}>
                          {q.quotes.status}
                        </span>
                ...
                <div className="p-10 text-center">
                  ...
                  <Link href="/app" className="mt-4 inline-block text-sm font-bold text-blue-600">Crear el primero →</Link>
                </div>
              )}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid gap-4">
              <Link href="/app/companies" className="group rounded-2xl border border-slate-200 bg-white p-4 shadow-sm hover:border-blue-200 hover:bg-blue-50/30 transition">
                <p className="font-bold text-slate-900 group-hover:text-blue-600">Mis Empresas</p>
                <p className="text-xs text-slate-500">Configura tus datos de emisor.</p>
              </Link>
              <Link href="/app/clients" className="group rounded-2xl border border-slate-200 bg-white p-4 shadow-sm hover:border-blue-200 hover:bg-blue-50/30 transition">
            ...
            <div className="grid gap-12 lg:grid-cols-2 lg:items-center lg:gap-16">
              <div className="text-center lg:text-left">
                ...
                <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600 lg:text-xl">
                  Genera propuestas comerciales profesionales con IA. Obtén análisis detallado, score de conversión y versiones optimizadas para cerrar más ventas.
                </p>
