...
function Step4Lines({ onPrev, data, updateData }: Omit<StepProps, 'onNext'>) {
  ...
  const calculateSubtotal = () => {
    return lines.reduce((acc, line) => {
      ...
      return acc + subtotal - (subtotal * discount / 100);
    }, 0);
  };
  ...
  const calculateTotal = () => {
    return lines.reduce((acc, line) => {
      const subtotal = line.quantity * line.unitPrice;
      const discount = line.discount || 0;
      const sub = subtotal - (subtotal * discount / 100);
        ...
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-white">
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="text-center">
              <h3 className="text-xl font-bold text-slate-900">Creando presupuesto...</h3>
              <p className="mt-2 text-sm text-slate-500">La IA está generando tu presupuesto profesional</p>
            </div>
          </div>
        </div>
      )}
